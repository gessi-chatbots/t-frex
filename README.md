...
*(Note: the file for extended pre-training **reviews-extended-pretraining.txt** is available in the latest release package as it exceeds the 100MB maximum size limit for GitHub storage. After downloading, place it on the ```data/T-FREX-EP``` folder).* 
